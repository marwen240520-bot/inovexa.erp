...
export default function ProfilePage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (filename) {
          ...
          showMessage("?? Image upload�e", "warning");
        }
      } else {
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showMessage(t("profile.passwordChanged") || "? Mot de passe chang�", "success");
      } else {
  ...
  const logoutAllDevices = async () => {
    if (confirm(t("profile.logoutAllWarning") || "?? Se d�connecter de tous les appareils ?")) {
      const token = localStorage.getItem("token");
      try {
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
          ...
          <div style={{ 
            ...
            {[
              ...
              { id: "security", label: t("profile.security") || "?? S�curit�", Icon: Icons.ShieldLock },
              { id: "activity", label: t("profile.activity") || "?? Activit�", Icon: Icons.BarChart2 },
            ].map(tab => (
              <button
                key={tab.id}
