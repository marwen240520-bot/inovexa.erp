...
// Composant SelectAllCheckbox local avec traduction
function LocalSelectAllCheckbox({ items, selectedIds, onSelect, onSelectAll, getItemId }) {
...
export default function ProductionPage() {
  ...
  const { theme } = useTheme(); // ⭐ AJOUT DU THÈME
  ...
  const fetchProductionOrders = async () => {
    ...
    try {
      ...
      const avgProgress = data.length > 0 ? Math.round(data.reduce((s, o) => s + (Number(o.progress) || 0), 0) / data.length) : 0;
  ...
  const deleteSelected = async () => {
    ...
    if (confirm(t("production.confirmBulkDelete").replace("{count}", selectedIds.length))) {
      const token = localStorage.getItem("token");
      ...
      fetchProductionOrders();
      setSelectedIds([]);
      showMessage(t("production.ordersDeleted").replace("{count}", selectedIds.length), "success");
    }
  };
  ...
  // Graphiques avec couleurs du thème
  ...
  // Cartes statistiques avec thème
  const statsCards = [
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.background, display: "flex" }}>
        ...
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <div style={{ 
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
              <div>
                <h1 style={{ color: theme.text, fontSize: "28px", margin: 0 }}>🏭 {t("common.production")}</h1>
              ...
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                <div style={{ display: "flex", gap: "8px" }}>
                  <button 
                    ...
                    style={{ padding: "8px 16px", background: viewMode === "list" ? theme.primary : theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "8px", color: "white", cursor: "pointer", transition: "all 0.2s" }}
                  >
                    📋 {t("production.listView") || "Liste"}
                  </button>
                  <button 
                    onClick={() => setViewMode("grid")} 
                    style={{ padding: "8px 16px", background: viewMode === "grid" ? theme.primary : theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "8px", color: "white", cursor: "pointer", transition: "all 0.2s" }}
                  >
                ...
                <button 
                  onClick={() => setModal({ open: true, editMode: false, form: { productName: "", quantity: 1, priority: "medium", assignedTo: "", startDate: "", status: "pending" } })} 
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "12px", marginBottom: "32px" }}>
            {statsCards.map((card, idx) => (
              <div key={idx} style={{ 
                ...
                animation: `fadeInUp 0.5s ease ${0.05 + idx * 0.02}s`,
                opacity: animateCards ? 1 : 0,
          ...
          <div style={{ 
            ...
            <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", marginBottom: "16px" }}>
              <input 
                ...
                style={{ flex: 2, padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text, transition: "border-color 0.2s" }}
              ...
              <select 
                ...
                style={{ padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text, minWidth: "150px", cursor: "pointer" }}
              >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "16px", flexWrap: "wrap", gap: "12px" }}>
              ...
              {selectedIds.length > 0 && (
                <button 
                  ...
                  style={{ background: "#c33", color: "white", border: "none", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", transition: "opacity 0.2s" }}
                    ...
                    <div style={{ display: "flex", gap: "12px", alignItems: "center", flexWrap: "wrap" }}>
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        <button 
                          ...
                          style={{ background: "#f59e0b", color: "white", border: "none", borderRadius: "8px", padding: "5px 10px", cursor: "pointer", transition: "opacity 0.2s" }}
                          onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                          onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
                          title={t("common.edit")}
                        >
                        ...
                        <button 
                          ...
                          style={{ background: "#c33", color: "white", border: "none", borderRadius: "8px", padding: "5px 10px", cursor: "pointer", transition: "opacity 0.2s" }}
                          onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                          onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
                  ...
                  <div>
                    ...
                    <div 
                      ...
                      onClick={() => {
                        ...
                        const newProgress = prompt(t("production.progressPrompt"), order.progress || 0);
                        if (newProgress !== null && !isNaN(newProgress) && newProgress >= 0 && newProgress <= 100) {
                          updateProgress(order.id, parseInt(newProgress));
                        }
                      }}
                    >
                      <div style={{ 
              ...
              {filteredOrders.map((order, idx) => (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button 
                      onClick={() => openEditModal(order)} 
                      style={{ flex: 1, padding: "8px", background: "#f59e0b", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", transition: "opacity 0.2s" }}
                      onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                      onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
                    >
                      ✏️ {t("common.edit")}
                    ...
                    <button 
                      onClick={() => deleteOrder(order.id)} 
                      style={{ flex: 1, padding: "8px", background: "#c33", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", transition: "opacity 0.2s" }}
                      onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                      onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
                    >
                      🗑️ {t("common.delete")}
      ...
      {/* Modal Ajout/Modification ordre */}
      {modal.open && (
        <div style={{ 
          position: "fixed", 
          top: 0, 
          ...
          <div style={{ background: theme.surface, padding: "32px", borderRadius: "24px", width: "500px", maxWidth: "90%", maxHeight: "90vh", overflowY: "auto", border: `1px solid ${theme.border}` }}>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <input 
                type="text" 
                placeholder={t("production.productName")} 
                value={modal.form.productName || ""} 
                onChange={e => setModal({ ...modal, form: { ...modal.form, productName: e.target.value } })} 
                style={{ width: "100%", padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text, transition: "border-color 0.2s" }}
                onFocus={(e) => e.currentTarget.style.borderColor = theme.primary}
                onBlur={(e) => e.currentTarget.style.borderColor = theme.border}
                autoFocus
              />
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <input 
                ...
                onChange={e => setModal({ ...modal, form: { ...modal.form, quantity: parseInt(e.target.value) || 1 } })} 
                style={{ width: "100%", padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text }} 
              />
            </div>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <select 
                value={modal.form.priority || "medium"} 
                onChange={e => setModal({ ...modal, form: { ...modal.form, priority: e.target.value } })} 
                style={{ width: "100%", padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text, cursor: "pointer" }}
              >
                <option value="low">🟢 {t("production.low")}</option>
                <option value="medium">🟡 {t("production.medium")}</option>
                <option value="high">🔴 {t("production.high")}</option>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <input 
                ...
                value={modal.form.assignedTo || ""} 
                onChange={e => setModal({ ...modal, form: { ...modal.form, assignedTo: e.target.value } })} 
                style={{ width: "100%", padding: "12px", background: theme.surfaceHover, border: `1px solid ${theme.border}`, borderRadius: "10px", color: theme.text }} 
              />
            </div>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button 
                onClick={modal.editMode ? updateProductionOrder : createProductionOrder} 
                style={{ flex: 1, padding: "12px", background: theme.primary, color: "white", border: "none", borderRadius: "10px", cursor: "pointer", fontWeight: "500", transition: "opacity 0.2s" }}
                onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
              >
              ...
              <button 
                ...
                style={{ flex: 1, padding: "12px", background: theme.surfaceHover, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: "10px", cursor: "pointer", transition: "opacity 0.2s" }}
                onMouseEnter={(e) => e.currentTarget.style.opacity = "0.8"}
                onMouseLeave={(e) => e.currentTarget.style.opacity = "1"}
              >
                {t("common.cancel")}
