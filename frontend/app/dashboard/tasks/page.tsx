...
export default function TasksPage() {
  ...
  const updateStatus = async (id, status) => {
    ...
    fetchTasks();
    setMessage(`✅ Tâche ${status === 'done' ? 'terminée' : 'en cours'}`);
      ...
      <div style={{ marginLeft: '280px', flex: 1, padding: '32px' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
            <div>
              <h1 style={{ color: 'white', fontSize: '28px' }}>✅ Gestion des tâches</h1>
              <p style={{ color: '#94a3b8' }}>Organisez et suivez vos tâches quotidiennes</p>
            </div>
            <button onClick={() => setModal({ open: true, form: { title: '', description: '', priority: 'medium', dueDate: '' } })} style={{ background: '#667eea', color: 'white', padding: '12px 24px', border: 'none', borderRadius: '12px', cursor: 'pointer' }}>
              + Nouvelle tâche
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            <div style={{ background: '#111', borderRadius: '20px', padding: '24px', border: '1px solid #222' }}>
              <h2 style={{ color: 'white', marginBottom: '16px' }}>📋 En cours ({pendingTasks.length})</h2>
              {pendingTasks.map(task => (
                <div key={task.id} style={{ background: '#1a1a1a', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                  ...
                  <p style={{ color: '#94a3b8', fontSize: '12px', marginTop: '8px' }}>{task.description}</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
                    <span style={{ color: '#666', fontSize: '10px' }}>Échéance: {new Date(task.dueDate).toLocaleDateString()}</span>
                    <button onClick={() => updateStatus(task.id, 'done')} style={{ background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', padding: '6px 12px', cursor: 'pointer' }}>
                      Terminer
                    </button>
                  </div>
                </div>
              ))}
              {pendingTasks.length === 0 && <p style={{ color: '#666', textAlign: 'center' }}>Aucune tâche en cours</p>}
            </div>
            ...
            <div style={{ background: '#111', borderRadius: '20px', padding: '24px', border: '1px solid #222' }}>
              ...
              {completedTasks.map(task => (
                <div key={task.id} style={{ background: '#1a1a1a', borderRadius: '12px', padding: '16px', marginBottom: '12px', opacity: 0.7 }}>
                  ...
                  <p style={{ color: '#666', fontSize: '12px', marginTop: '8px' }}>{task.description}</p>
                </div>
              ))}
        ...
        {modal.open && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: '#111', padding: '32px', borderRadius: '24px', width: '450px' }}>
              ...
              <input type="date" placeholder="Date d'échéance" value={modal.form.dueDate || ''} onChange={e => setModal({ ...modal, form: { ...modal.form, dueDate: e.target.value } })} style={{ width: '100%', padding: '12px', marginBottom: '20px', background: '#1a1a1a', border: '1px solid #333', borderRadius: '10px', color: 'white' }} />
