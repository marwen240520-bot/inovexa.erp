...
export class WorkOrdersService {
  constructor(
    ...
    private repository: Repository<WorkOrders>,
  ) {}
  ...
  async findOne(id: string): Promise<WorkOrders | null> {
    return this.repository.findOne({ where: { id } });
  }

  async create(data: Partial<WorkOrders>): Promise<WorkOrders> {
    const item = this.repository.create(data);
    return this.repository.save(item);
  }

  async update(id: string, data: Partial<WorkOrders>): Promise<WorkOrders | null> {
